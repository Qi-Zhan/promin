...
@pm.register_type(
    layout={"name": "row", "params": {"wrap": True, "columns": 3}},
    shape="box",
    label="summary",
...
def test_custom_layout_plugin_applies_coordinates():
    def vertical(ctx: pm.LayoutContext) -> pm.LayoutResult:
    ...
    pm.register_layout("vertical_test", vertical)

    @pm.register_type(
        layout={"name": "vertical_test", "params": {}},
        shape="circle",
        label="key",
        edges=["left", "right"],
    )
    ...
    @pm.register_type(layout={"name": "row", "params": {}}, shape="box", label="v")
    class _Leaf:
